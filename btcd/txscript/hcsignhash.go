...
// HCSigHashType represents hash type bits at the end of a signature.
type HCSigHashType byte

// Hash type bits from the end of a signature.
const (
	HCSigHashAll          HCSigHashType = 0x1
	HCSigHashNone         HCSigHashType = 0x2
	HCSigHashSingle       HCSigHashType = 0x3
	HCSigHashAnyOneCanPay HCSigHashType = 0x80
...
// SigHCHashSerType represents the serialization type used when calculating
// signature hashes.
//
...
func sigHashPrefixSerializeSize(hashType HCSigHashType, txIns []*wire.HCTxIn, txOuts []*wire.HCTxOut, signIdx int) int {
	...
	for txOutIdx, txOut := range txOuts {
		...
		if hashType&sigHCHashMask == HCSigHashSingle && txOutIdx != signIdx {
			pkScript = nil
		}
...
// the overall signature hash.
func sigHashWitnessSerializeSize(hashType HCSigHashType, txIns []*wire.HCTxIn, signScript []byte) int {
	// 1) 4 bytes version/serialization type
	// 2) number of inputs varint
	// 3) per input:
...
func calcHCSignatureHash(prevOutScript []parsedOpcode, hashType HCSigHashType, tx *wire.HCMsgTx, idx int, cachedPrefix *chainhash.Hash) ([]byte, error) {
	...
	if hashType&sigHCHashMask == HCSigHashSingle && idx >= len(tx.HCTxOut) {
		str := fmt.Sprintf("attempt to sign single input at index %d "+
			">= %d outputs", idx, len(tx.HCTxOut))
	...
	txIns := tx.HCTxIn
	signTxInIdx := idx
	if hashType&HCSigHashAnyOneCanPay != 0 {
		txIns = tx.HCTxIn[idx : idx+1]
		signTxInIdx = 0
	}
	...
	if chaincfg.SigHashOptimization && cachedPrefix != nil &&
		hashType&sigHCHashMask == HCSigHashAll &&
		hashType&HCSigHashAnyOneCanPay == 0 {
		...
		txOuts := tx.HCTxOut
		switch hashType & sigHCHashMask {
		case HCSigHashNone:
			txOuts = nil
		case HCSigHashSingle:
			txOuts = tx.HCTxOut[:idx+1]
		default:
			fallthrough
		case HCSigHashAll:
		...
		version := uint32(tx.Version) | uint32(SigHCHashSerializePrefix)<<16
		...
		for txInIdx, txIn := range txIns {
			...
			prevOut := &txIn.PreviousHCOutPoint
			...
			if (hashType&sigHCHashMask == HCSigHashNone ||
				hashType&sigHCHashMask == HCSigHashSingle) &&
				txInIdx != signTxInIdx {
		...
		for txOutIdx, txOut := range txOuts {
			...
			value := txOut.Value
			pkScript := txOut.PkScript
			if hashType&sigHCHashMask == HCSigHashSingle && txOutIdx != idx {
				value = -1
				pkScript = nil
			}
...
// providing the prefix hash to be reused in the case of SigHashAll without the
// SigHashAnyOneCanPay flag set.
func CalcHCSignatureHash(script []byte, hashType HCSigHashType, tx *wire.HCMsgTx, idx int, cachedPrefix *chainhash.Hash) ([]byte, error) {
	pops, err := parseScript(script)
	if err != nil {
		return nil, err
	}

	return calcHCSignatureHash(pops, hashType, tx, idx, cachedPrefix)
}
