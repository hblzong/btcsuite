...
import (
	...
	"github.com/btcsuite/btcutil/bech33"
...
func DecodeHCAddress(addr string, defaultNet *chaincfg.Params) (Address, error) {
	decoded, netID, err := base58.CheckHCDecode(addr)
	if err != nil {
	...
	//fmt.Println("netID = ", netID)
	switch len(decoded) {
	case ripemd160.Size: // P2PKH or P2SH
		return newHCAddressPubKeyHash(decoded, netID, defaultNet.PubKeyHashAddrID)
		// isP2PKH := netID[0] == defaultNet.PubKeyHashAddrID
		// isP2SH := netID[0] == defaultNet.ScriptHashAddrID
...
func newHCAddressPubKeyHash(pkHash []byte, hcNetID [2]byte, netID byte) (*AddressPubKeyHash, error) {
	// Check for a valid pubkey hash length.
	if len(pkHash) != ripemd160.Size {
		return nil, errors.New("pkHash must be 20 bytes")
	}
	addr := &AddressPubKeyHash{hcNetID: hcNetID, netID: netID}
...
// bytes.
func NewAddressWithWif(wif *WIF) (*AddressPubKeyHash, error) {
	pk := wif.SerializePubKey()
	fmt.Println("pk : ", hex.EncodeToString(pk))
	//	address, err := btcutil.NewAddressPubKeyHash( (*btcec.PublicKey)
	//	btcutil.Hash160(pk), &chaincfg.TestNet3Params)
	return NewAddressPubKeyHash(Hash160(pk), &chaincfg.MainNetParams)
}
...
// structure from a string encoding where the identifer byte is already
// known.
func GenerateAddressPubKeyHash(pkHash []byte, netID byte) (*AddressPubKeyHash, error) {
	// Check for a valid pubkey hash length.
	if len(pkHash) != ripemd160.Size {
		return nil, errors.New("pkHash must be 20 bytes")
	}
