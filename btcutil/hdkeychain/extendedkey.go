...
func (k *ExtendedKey) XPrv() (bip32.XPrv, error) {
	if !k.isPrivate {

		return bip32.XPrv{}, ErrNotPrivExtKey
	}
	seed:=make([]byte, 32)
	copy(seed, k.key)
	return bip32.NewRootXPrv(seed), nil
}

func (k *ExtendedKey) PrivKeySecp256k1() (secp256k1.PrivKeySecp256k1, error) {
	if !k.isPrivate {
		return secp256k1.PrivKeySecp256k1{}, ErrNotPrivExtKey
	}
	return secp256k1.GenPrivKeySecp256k1(k.key), nil

}

func (k *ExtendedKey) IOSTKey() []byte {
	seed := make([]byte, 32)
	copy(seed,k.key)
	return seed
}

func (k *ExtendedKey) AvaxPriv() (*crypto.PrivateKeySECP256K1R, error) {
	if !k.isPrivate {
		return &crypto.PrivateKeySECP256K1R{}, ErrNotPrivExtKey
	}
	factory := crypto.FactorySECP256K1R{}
	sk, err := factory.NewPrivateKeyByByte(k.key)
	if err != nil {
		return &crypto.PrivateKeySECP256K1R{}, ErrNotPrivExtKey
	}
...
// returned if this should occur, so the caller must check for it and generate a
// new seed accordingly.
func NewSecretKey(secretKey []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	// Per [BIP32], the seed must be in range [MinSeedBytes, MaxSeedBytes].
